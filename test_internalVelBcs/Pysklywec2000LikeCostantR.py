...
# ### Note
# 
# Similar model to Pysklywec2000 and Pysklywec2010, but with extented box and boudary conditions.
# 
# 
# ### Ref
...
# - Pysklywec, R. N., Gogus, O., Percival, J., Cruden, A. R., & Beaumont, C. (2010). Insights from geodynamical modeling on possible fates of continental mantle lithosphere: Collision, removal, and overturn. Canadian Journal of Earth Sciences, 47(4), 541-563.
...
Model.outputDir= "op_Pysklywec2000Like"
...
Z_oml = -120. * u.kilometer
Z_iml = -120. * u.kilometer
Z_aml = -120. * u.kilometer
...
SL_ws =  10 * u.kilometer 
X0_ws =  900 * u.kilometer - SL_ws/2.
X1_ws = 900 * u.kilometer + SL_ws/2. 
Y1_ws = -50 * u.kilometer + SL_ws/2.
Y0_ws = -50 * u.kilometer - SL_ws/2. 
...
oc_Shape1 = GEO.shapes.Polygon([(X0_oc,  h_o),
                               (X1_oc,h_o),
                               (X1_oc,  Z_oc),
                               (X0_oc, Z_oc)])

oc_Shape2 = GEO.shapes.Polygon([(X2_oc,  h_o),
                               (X3_oc,h_o),
                               (X3_oc,  Z_oc),
                               (X2_oc, Z_oc)])
...
ic_Shape1 = GEO.shapes.Polygon([(X0_ic,  h_i),
                               (X1_ic,  h_i),
                               (X1_ic, Z_ic),
                               (X0_ic, Z_ic)])
ic_Shape2 = GEO.shapes.Polygon([(X2_ic,  h_i),
                               (X3_ic,  h_i),
                               (X3_ic, Z_ic),
                               (X2_ic, Z_ic)])
...
ac_Shape = GEO.shapes.Polygon([(X0_ac,  h_a),
                               (X1_ac,  h_a),
                               (X1_ac, Z_ac),
                               (X0_ac, Z_ac)])
...
ws_Shape = GEO.shapes.Box(bottom=Y0_ws, top=Y1_ws, minX=X0_ws, maxX=X1_ws)
...
ws = Model.add_material(name="Week seed", shape=ws_Shape)
...
ws.density = 3300 * u.kilogram / u.metre**3
...
ws.viscosity = 1e20 * u.pascal * u.second


# ml0.plasticity = GEO.DruckerPrager(name="plasticity_ml",cohesion=20.0 * u.megapascal,
#                                cohesionAfterSoftening=10.0 * u.megapascal,
#                                frictionCoefficient = np.tan(np.radians(15.0)), 
#                                frictionAfterSoftening = np.tan(np.radians(2.0)),
#                                epsilon1=0.5,
#                                epsilon2=1.5)
...
axis_ic = np.where((Model.mesh.data[:,0]<=GEO.nd(X1_ic)+dx/4)&(Model.mesh.data[:,0]>=GEO.nd(X0_ic)-dx/4)&(Model.mesh.data[:,1]>=(GEO.nd(Z_iml)-dy/4)))
IndexSet_icL = find_IndexSet(axis_ic)

axis_ic = np.where((Model.mesh.data[:,0]<=GEO.nd(X3_ic)+dx/4)&(Model.mesh.data[:,0]>=GEO.nd(X2_ic)-dx/4)&(Model.mesh.data[:,1]>=(GEO.nd(Z_iml)-dy/4)))
...
Model.set_velocityBCs(left=[0., None], right=[0., None], top=[None, None], bottom=[None,0.],nodeSets=[(IndexSet_icL,[velocity,0.]),(IndexSet_icR,[-velocity,0.])])
...
# Fig = vis.Figure(resolution=(1000,600),rulers=True,margin = 100,rulerticks=7,quality=3,clipmap=False)
# Fig.Surface(Model.mesh, Model.pressureField) #fn_size=4.0,discrete=True,colourBar=False)
# Fig.show()
# Fig.save(Model.outputDir+"/pressure_0.png")
